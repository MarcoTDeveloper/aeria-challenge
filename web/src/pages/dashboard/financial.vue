...
<template>
  <div class="tw-justify-center tw-items-center tw-flex tw-flex-col md:tw-flex-row tw-gap-4">
    ...
    <div class="tw-bg-indigo-400 tw-text-white tw-rounded-md tw-p-6 tw-w-full tw-mx-auto md:tw-m-0 md:tw-w-80">
      <div class="tw-flex tw-items-center tw-justify-between tw-mb-3">
        <span>Saldo atual</span>
        <img class="tw-w-6 tw-h-6" src="../../../static/dollar.svg">
      </div>

      <span class="tw-font-bold tw-text-2xl">{{ formatMoney.format(Number(result?.balance)) }}</span>
    </div>
  
    <div class="tw-border-2 tw-border-indigo-400 tw-text-indigo-400 tw-rounded-md tw-p-6 tw-w-full tw-mx-auto md:tw-m-0 md:tw-w-80">
      <div class="tw-flex tw-items-center tw-justify-between tw-mb-3">
        <span>A receber</span>
        <img class="tw-w-6 tw-h-6" src="../../../static/circle-arrow-up.svg">
      </div>

      <span class="tw-font-bold tw-text-2xl">{{ formatMoney.format(Number(result?.receiveTransaction)) }}</span>
    </div>
    ...
    <div class="tw-border-2 tw-border-indigo-400 tw-text-indigo-400 tw-rounded-md tw-p-6 tw-w-full tw-mx-auto md:tw-m-0 md:tw-w-80">
      ...
      <span class="tw-font-bold tw-text-2xl">{{ formatMoney.format(Number(result?.payTransactions)) }}</span>
    </div>
